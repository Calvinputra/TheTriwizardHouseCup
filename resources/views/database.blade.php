    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      ...
      <div class="mb-6 flex justify-between items-center">
        <div>
          ...
          <p class="text-blue-100/70 text-sm mt-1">Kelola House untuk setiap peserta dan lihat statistiknya di bawah.</p>
        </div>
        <a href="{{ route('home') }}" 
            ...
            @if($members->count() > 0)
              <ul class="list-disc pl-5 space-y-1 text-blue-100/80">
                @foreach($members as $m)
                  <li>{{ $m->name }} <span class="text-blue-200/60 text-xs">({{ $m->email }})</span></li>
                @endforeach
              </ul>
            @else
